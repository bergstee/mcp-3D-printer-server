...
import { STLManipulator, ProgressCallback } from '../stl/stl-manipulator.js';
import fs from 'fs';
...
async function testSTLManipulator() {
  try {
    ...
    const modifiedStlPath = await manipulator.modifySection(
      stlPath,
      'top',
      {
        type: 'scale',
    ...
    const visualizationPath = await manipulator.generateVisualization(
      stlPath,
      400, 
      400,
      progressCallback
